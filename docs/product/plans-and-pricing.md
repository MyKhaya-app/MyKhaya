...
The table above didn't change — this was a correction pass making sure the product actually behaves this way everywhere, after a review found a Free Home could still reach Family functionality through a few surfaces: the Home screen's "Invite family" action and the Family page's "Add member" button appeared regardless of plan, the Routines screen let a Free user pick "Household" only to have it rejected on save, and — the two genuine gaps — accepting a stale invitation issued while a Home was still Family, and adding a child profile directly, could each still grow a Free Home past one person. All are now closed: the two backend gaps were fixed outright, and every screen now checks the Home's actual plan before showing an action it can't complete, replacing it with a plain "Available with MyKhaya Family" note where relevant instead of letting someone discover the limit only after trying. Full detail in `docs/architecture/commercial-entitlements.md`'s "Free plan enforcement pass" section.
