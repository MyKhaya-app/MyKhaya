...
## Free plan enforcement pass, part 2: shared events were an indirect restriction

Full architecture in `docs/architecture/commercial-entitlements.md`'s "Free plan enforcement pass, part 2". The finding: a Free Home couldn't create a multi-participant event, but only because `home.max_members` gave it no one else to assign — `events.shared.enabled` itself was never actually checked anywhere. A downgraded Family Home that kept several existing members (the member limit never evicts anyone) had no entitlement check standing between it and unlimited new shared-event creation.

`routers.calendar.create_event`/`update_event` now check `events.shared.enabled` directly, using the same "genuinely new participant" transition-safe test already established for household routines and Extended Family relationship changes: a request only trips the check when it introduces a participant not already on the event *and* the result is still multi-person. Removing a participant, or resubmitting the exact same participant set while editing other fields, never trips it — this is what lets a historical shared event survive a downgrade with its data intact, per the standing non-destructive-downgrade rule. Verified directly: attempting to add a new participant to a preserved historical shared event on a downgraded Free Home is refused (`plan_feature_unavailable`); attempting to remove one, or re-save unrelated fields with the same participants, succeeds.

`members.external_invites.enabled` had the same shape of gap: Extended Family/Friend is a genuinely reachable relationship type (not a stub — `PermissionProfile.explicit_sharing`, selectively granted `shared_resources`), reachable at both invitation creation and the member-relationship-change endpoint (`PATCH /groups/{id}/members/{user_id}`), and neither previously checked this entitlement at all — the relationship-change endpoint in particular was a completely unchecked path to grant Extended Family/Friend status (and its `shared_resources`) to any existing member. Both endpoints now check it, with the same transition-safe exemption for members who already hold that relationship.

`family_plans.enabled`/`support.priority.enabled` were re-confirmed to have zero executable feature surface anywhere in the codebase (no router, task, serializer, or component) — nothing to enforce, nothing invented. They remain pinned by the existing capability-matrix tests in `test_entitlements.py`, now explicitly documented as "contract only" rather than "deferred enforcement" (the Platform Control Centre distinguishes the two with separate badges) since there is no feature to defer enforcement *on*.

## Event categories were enforced against the wrong resource

Full architecture in `docs/architecture/commercial-entitlements.md`'s "Event categories are CalendarEventLabel, not HomeCalendar". A production-facing entitlement bypass: `calendar.max_categories` was correctly and thoroughly enforced against `HomeCalendar` (Phase 6), but the actual user-facing "category" feature — Settings -> Home settings' "Calendars & categories" page, which manages `CalendarEventLabel` rows — had no enforcement at all. Every new Home was seeded with all 7 default categories active simultaneously, so a Free Home could rename, recolour, and freely use all 7 as fully active categories with no plan restriction whatsoever, and could create unlimited further ones via `POST /event-labels`.

Fixed at three points: (1) seeding — only the first default label now starts active on a new Home, at both seeding call sites (`routers.groups.create_group`, `routers.calendar._ensure_home_calendar`); (2) `create_label`/`update_label` now enforce `calendar.max_categories` race-safe, transition-safe on activation (matching the pattern already established for routines/members/relationships); (3) `create_event`/`update_event` now reject assigning a label the classify-ordered-resources check marks `read_only_due_to_plan` — closing the direct-API path where a Free Home, having preserved several active labels from a Family downgrade, could otherwise still assign a locked one to a brand-new event even though the management page would show it as locked.

No historical data was touched — deactivating a label is never automatic; downgrade only changes which label is *newly* usable, computed fresh on every read from current entitlement + current label state, identically to how `HomeCalendar.commercial_access` has worked since Phase 6.
