...
Concretely, "platform catalogues" means `mykhaya.module_registry.feature_modules()` (backing `GET /platform/modules`, PCC's global module screen) and `household_modules()` (backing the Home Admin Module Management and consumer navigation listings) — both exclude hidden modules structurally (Phase 3A), not just by convention. `PUT /platform/modules/{key}` additionally refuses to write a lifecycle change for a module whose *static registry* `release_state` is `hidden`, so a stale or forced `FeatureFlag` database row can never make a hidden module appear released through this control either — promoting a module out of `hidden` is a code/release change to `module_registry.py`, never an operator action.
