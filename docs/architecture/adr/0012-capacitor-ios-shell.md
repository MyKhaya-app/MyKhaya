...
## Secure storage: deferred in Phase 3, resolved in Phase 4

Phase 2 shipped only `InMemoryNativeSessionStore` — adequate for testing the auth flow logic, but not for real persistent login (a token that must survive app restart needs iOS Keychain, not process memory). Phase 3 reviewed two candidate plugins without being able to verify their actual Keychain-accessibility-flag behaviour from a non-Mac sandbox, and deliberately shipped nothing rather than guess.
