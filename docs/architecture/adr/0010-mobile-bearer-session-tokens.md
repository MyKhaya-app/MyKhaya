...
**Status:** Accepted and implemented (revision 2 approved by Anthony). The `apps/mobile` native client this ADR was written for is retired by [ADR 0011](./0011-single-pwa-retire-mobile-app.md); the bearer-token mechanism described below remained in place server-side as a generically useful non-cookie auth transport, unused by any client from the point `apps/mobile` was retired until the iOS/Capacitor readiness work picked it back up — `packages/api-client`'s `NativeMyKhayaClient` (login, child login, rotation, logout, the single native request wrapper, and an in-memory `NativeSessionStore` pending a future Capacitor Keychain adapter) now consumes it, and `POST /auth/mobile/child/login` was added alongside the original three endpoints so a managed-Child account has the same bearer path an adult account does — the same Session-row-with-a-`kind` model this ADR already established for cookies, not a second child-auth mechanism. Written in response to a security design checkpoint raised while building the mobile Calendar foundation. Revision 2 incorporated review feedback from Anthony (with architectural input from a separate reviewer) on the original proposal; every numbered change below traces to a specific point raised in that review. Implementation: `apps/api/mykhaya/security.py`, `dependencies.py`, `routers/auth.py`, `schemas.py`, `apps/api/tests/test_mobile_auth.py`, `packages/api-client/src/native-client.ts`, `native-session-store.ts`, `native-config.ts`. One documented deviation - see "Deviation from this ADR" at the end.
