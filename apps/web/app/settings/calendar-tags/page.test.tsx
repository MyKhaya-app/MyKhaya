...
import CalendarTagsPage from "./page";
...
//
// Moved here from app/settings/home/page.test.tsx when Calendar Tags
// management became its own More destination (previously inlined into
// Home settings) — see components/calendar-tags-manager.tsx.

vi.mock("next/navigation", () => ({
  useRouter: () => ({ replace: vi.fn(), push: vi.fn() }),
  usePathname: () => "/settings/calendar-tags",
}));

function freeHome(overrides: Partial<Home> = {}): Home {
  return {
    id: "home-1",
    name: "Hales Home",
    role: "owner",
...
let activeHomeValue = freeHome();
vi.mock("@/components/use-active-home", () => ({
  useActiveHome: () => ({
    activeHome: activeHomeValue,
    activeHomeId: activeHomeValue.id,
    homes: [activeHomeValue],
...
describe("Calendar tags — locked states", () => {
  it("shows only one Calendar Tag as Active/manageable on Free, the rest locked", async () => {
    ...
    await screen.findByRole("heading", { name: /calendar tags/i, level: 2 });
...
describe("Calendar tags — permission gating", () => {
  it("shows a read-only notice, not the manager, without calendar.edit_all", async () => {
    ...
    await screen.findByText(/don.t currently have permission/i);
    expect(
      screen.queryByRole("heading", { name: /calendar tags/i, level: 2 }),
...
// customises its colour. Its name is a fixed product concept: this page
// must never expose a way to rename it.
describe("Calendar tags — Home calendar colour", () => {
  beforeEach(() => {
    (api.listLabels as ReturnType<typeof vi.fn>).mockResolvedValue(sevenSeededLabels);
    (api.billingStatus as ReturnType<typeof vi.fn>).mockResolvedValue({
      category_usage: { count: 1, limit: 1, over_limit: false },
...
describe("Calendar tags — deleting a Calendar Tag", () => {
