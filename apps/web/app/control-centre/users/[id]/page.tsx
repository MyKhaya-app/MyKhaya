...
export default function PlatformUserDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <PlatformShell><main className="platform-page"><div className="platform-heading"><div><p>User account</p><h1>{data?.display_name ?? "User"}</h1></div><span>{data?.email}</span></div>{message && <p className="notice" role="status">{message}</p>}{data && <>
    <section className="record-card"><h2>Account metadata</h2><dl><div><dt>Status</dt><dd>{data.active ? "Active" : "Suspended"}</dd></div><div><dt>Email verification</dt><dd>{data.verified ? "Verified" : "Unverified"}</dd></div><div><dt>Created</dt><dd>{new Date(data.created_at).toLocaleString()}</dd></div><div><dt>Last login</dt><dd>{data.last_login_at ? new Date(data.last_login_at).toLocaleString() : "No login recorded"}</dd></div></dl></section>
