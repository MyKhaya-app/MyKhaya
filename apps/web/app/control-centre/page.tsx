...
export default function PlatformOverview() {
  ...
  return (
    <PlatformShell>
      <CcPage className="platform-overview">
        <CcPageHeader
        ...
        />
        {error && <CcNotice tone="error">Live platform state is unavailable: {error}</CcNotice>}
        {!data ? (
          ...
          <>
            ...
            <div className="overview-grid">
                ...
                <div className="summary-groups">
                  <div>
                    ...
                    <dl>
                      ...
                      <div>
                        <dt>Verified</dt>
                        <dd>{data.users.verified}</dd>
                      </div>
                      <div>
                        <dt>Unverified</dt>
                        <dd>{data.users.unverified}</dd>
                      </div>
                    </dl>
                  </div>
                  <div>
                    ...
                    <dl>
                      <div>
                        <dt>Active</dt>
                        <dd>{data.homes.active}</dd>
                      </div>
                      <div>
                        <dt>Suspended</dt>
                        <dd>{data.homes.suspended}</dd>
                      </div>
                      <div>
                        <dt>Queue depth</dt>
                        <dd>{data.operations.queue_depth ?? "Unavailable"}</dd>
                      </div>
                    </dl>
                  </div>
                </div>
                ...
                <div className="health-summary">
                  {data.health.map((item) => (
                    <div key={item.service}>
                      <span
                        className={`health-mark state-${item.state.toLowerCase().replace(" ", "-")}`}
                        aria-hidden="true"
                      />
              ...
              <section className="overview-panel">
                ...
                <dl>
                  <div>
                    <dt>Failed logins, 24 hours</dt>
                    <dd>{data.security.failed_logins_24h}</dd>
                  </div>
                  <div>
                    <dt>Locked accounts</dt>
                    <dd>{data.security.locked_accounts}</dd>
                  </div>
                  <div>
                    <dt>Active administrator sessions</dt>
                    <dd>{data.security.active_administrator_sessions}</dd>
                  </div>
                  <div>
                    <dt>Administrator MFA coverage</dt>
                    <dd>
                      {data.security.administrators_with_mfa} of {data.security.active_administrators}
                    </dd>
                  </div>
                </dl>
              </section>
            ...
            {Object.keys(data.deployment).length > 0 && (
