...
/** Reuses the existing GET /communications/diagnostics endpoint rather than
 *  introducing a parallel delivery-log store — this is a differently-framed
 *  view onto the same durable data the Communications > Diagnostics page
 *  already exposes. Never renders auth secrets, tokens, reset links or
 *  verification codes — that endpoint already sanitises failure reasons and
 *  never returns raw content. */
export default function NotificationDeliveryLogsPage() {
  const [items, setItems] = useState<DeliveryEntry[]>([]);
  ...
  const search = useCallback(async (params: URLSearchParams, page: number, append: boolean) => {
    ...
    try {
      ...
      const response = await platformApi.get<{ items: DeliveryEntry[]; next_page: number | null }>(
  ...
  const columns: CcTableColumn<DeliveryEntry>[] = [
  ...
  return (
    <PlatformShell>
      <CcPage wide>
        <CcPageHeader eyebrow="Notifications" title="Delivery Logs" description="Filter delivery attempts by status, channel, type or recipient." />
        <NotificationsSubNav />
        ...
        <CcCard title="Search deliveries" icon={Search}>
          <form onSubmit={onSearch}>
            ...
            <CcActionBar
              actions={[{ key: "search", label: loading ? "Searching…" : "Search", variant: "primary", type: "submit", disabled: loading }]}
            />
          </form>
        </CcCard>
