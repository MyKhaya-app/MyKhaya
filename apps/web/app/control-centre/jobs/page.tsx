...
type Job = { id: string; job_type: string; state: string; created_at: string; started_at: string | null; completed_at: string | null; duration_ms: number | null; retry_count: number; safe_failure_message: string | null; occurrence_id: string | null; scheduled_for: string | null };
...
export default function JobsPage() {
  ...
  return <PlatformShell><main className="platform-page">
    ...
    {!data ? <p role="status">Loading queue and worker state…</p> : <>
      ...
      <section className="overview-panel job-runtime"><h2>Runtime</h2><dl><div><dt>Scheduler tick</dt><dd>{relativeTime(typeof summary?.scheduler_heartbeat === "string" ? summary.scheduler_heartbeat : null)}</dd></div><div><dt>Worker heartbeat</dt><dd>{relativeTime(typeof summary?.worker_heartbeat === "string" ? summary.worker_heartbeat : null)}</dd></div><div><dt>Next scheduled execution</dt><dd>{readableDate(typeof summary?.next_scheduled_execution === "string" ? summary.next_scheduled_execution : null)}</dd></div><div><dt>Last successful execution</dt><dd>{relativeTime(typeof summary?.last_successful_execution === "string" ? summary.last_successful_execution : null)}</dd></div></dl></section>
      ...
      <section><h2>Recent executions</h2>{data.items.length === 0 ? <p className="quiet-state">No jobs have executed yet.</p> : <div className="table-scroll" tabIndex={0}><table><thead><tr><th>Job</th><th>Scheduled for</th><th>State</th><th>Started</th><th>Duration</th><th>Attempt</th><th>Occurrence</th><th>Failure</th><th>Action</th></tr></thead><tbody>{data.items.map((job) => <tr key={job.id}><td>{titleCase(job.job_type)}</td><td>{job.scheduled_for ?? "—"}</td><td><span className={`state-label state-${job.state}`}>{titleCase(job.state)}</span></td><td>{readableDate(job.started_at)}</td><td>{job.duration_ms === null ? "—" : `${job.duration_ms} ms`}</td><td>{job.retry_count}</td><td title={job.occurrence_id ?? undefined}>{job.occurrence_id ? job.occurrence_id.slice(-18) : "—"}</td><td>{job.safe_failure_message ?? "—"}</td><td>{job.state === "failed" ? <button disabled={reason.trim().length < 10} onClick={() => retry(job)}>Retry</button> : "—"}</td></tr>)}</tbody></table></div>}</section>
    </>}
  </main></PlatformShell>;
}
