...
export default function DemoTestHomesPage() {
  ...
  const load = async () => { try { setRows(await platformApi.get<ManagedDemoHome[]>("/demo-test-homes")); setError(""); } catch (error) { setError(safeError(error, "Unable to load managed Demo/Test Homes.")); } };
  useEffect(() => { void load(); }, []);
