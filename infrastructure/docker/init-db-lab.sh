...
# Lab-only counterpart to init-db.sh. Keep the database name explicit so this
# script cannot accidentally grant the Lab application access to another DB.
psql --set=ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname "$POSTGRES_DB" \
  ...
  --set=runtime_password="$MYKHAYA_DB_PASSWORD" <<-'SQL'
    ...
    ALTER DATABASE mykhaya_lab OWNER TO mykhaya_migrator;
    GRANT CONNECT ON DATABASE mykhaya_lab TO mykhaya_app;
